...
<div style="max-width:580px;margin:40px auto;background:#fff;border:1px solid #E2E8F0;border-radius:16px;overflow:hidden">
  <div style="background:linear-gradient(135deg,#7C3AED 0%,#7C3AED 100%);padding:26px 30px">
  ...
  <div style="padding:26px 30px">
    ...
    <a href="{report_url}" style="display:inline-block;background:#7C3AED;color:#fff;text-decoration:none;padding:11px 26px;border-radius:10px;font-weight:700;font-size:.88rem">View Full Report &rarr;</a>
...
<div style="max-width:560px;margin:40px auto;background:#fff;border:1px solid #E2E8F0;border-radius:16px;overflow:hidden">
  ...
  <div style="padding:26px 30px">
    ...
    <a href="{APP_BASE_URL}" style="display:inline-block;background:#7C3AED;color:#fff;text-decoration:none;padding:11px 26px;border-radius:10px;font-weight:700;font-size:.88rem">Run Your First Check &rarr;</a>
    <p style="font-size:.78rem;color:#64748B;margin-top:22px;line-height:1.5">
      Questions? Reply to <a href="mailto:support@organicwebchecker.com" style="color:#7C3AED">support@organicwebchecker.com</a> — we&rsquo;re here to help.
    </p>
...
GLOBAL_CSS = """
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .status-done    { background: #E0F7E9; color: #3DAD62; border-color: #A8EFC0; }
  ...
  .section-label.green  { color: #3DAD62; }
  ...
  .green  .badge { background: #E0F7E9; border-color: #A8EFC0; color: #3DAD62; }
  ...
  .clean     { color: #3DAD62; font-weight: 700; padding: 14px 0; }
  ...
  .ps-done    .ps-num { background: #E0F7E9; color: #3DAD62; border: 1px solid #86EFAC; }
  ...
  .auth-msg.success { background: #F0FDF4; border: 1px solid #86EFAC; color: #3DAD62; }
  .nav-user-email {
  ...
  .next-avail-box:hover { background: linear-gradient(135deg,#E0F7E9,#A8EFC0); }
  .next-avail-label { font-size: .75rem; font-weight: 700; color: #2D8049; text-transform: uppercase; letter-spacing: .06em; }
  .next-avail-time  { font-size: 1rem; font-weight: 800; color: #2D8049; margin-top: 2px; }
  .next-avail-arrow { font-size: 1.2rem; color: #57C47A; flex-shrink: 0; }
  ...
  .slot-btn.avail    { background: #F0FDF4; border-color: #86EFAC; color: #3DAD62; }
  .slot-btn.avail:hover { background: #E0F7E9; border-color: #4ADE80; }
  ...
  .badge-scheduled  { background: #F0FDF4; border-color: #86EFAC; color: #3DAD62; }
...
{% else %}
  ...
  <div class="report-header">
    ...
    <div style="margin-top:6px">
      ...
      {% else %}
        <span style="font-size:.68rem;padding:2px 9px;border-radius:8px;background:#E0F7E9;color:#3DAD62;border:1px solid #A8EFC0;font-weight:700">&#10003; Live OID data</span>
      {% endif %}
    </div>
    </div>
      ...
      {% if report.get('scope') %}
        {% for s in report.scope %}
          <span style="display:inline-block;font-size:.68rem;padding:2px 8px;border-radius:6px;margin-right:4px;font-weight:700;
            ...
            {% elif s == 'CROPS' %}background:#F0FDF4;color:#3DAD62;border:1px solid #A8EFC0
...
<head>
  ...
  .chip-green   { background: #E0F7E9; color: #3DAD62; border-color: #86EFAC; }
...
<div class="account-wrap">
  ...
  <div id="acctLoggedIn" style="display:none">
    ...
    <div class="card" style="max-width:540px;margin:0 auto">
      ...
      <div id="apiKeyNewBox" style="display:none;padding:14px;background:#F0FDF4;border:1px solid #A8EFC0;border-radius:10px;margin-bottom:16px;font-size:.82rem">
        <strong style="color:#3DAD62">Key created — copy it now, it won&rsquo;t be shown again:</strong><br>
        <div style="display:flex;gap:8px;margin-top:8px;align-items:center">
          <code id="apiKeyNewValue" style="background:#E0F7E9;padding:6px 10px;border-radius:6px;font-size:.78rem;word-break:break-all;flex:1"></code>
...
<h2 style="color:#7C3AED;font-size:1.2rem;margin-bottom:8px">Password Reset</h2>
<p style="color:#64748B;font-size:.9rem;line-height:1.6;margin-bottom:24px">
  You requested a password reset for your Organic Web Checker account.
  Click the button below — the link expires in 1 hour.
</p>
<a href="{reset_url}" style="display:inline-block;background:#7C3AED;color:#fff;text-decoration:none;padding:12px 28px;border-radius:10px;font-weight:700;font-size:.9rem">Reset Password</a>
...
def history():
    ...
    def _row(j):
        ...
        md_link  = f'<a class="view-btn" href="/job/{j["id"]}/download/md" style="color:var(--cyan);border-color:rgba(0,229,204,.2)">&#8595; .md</a>' if not j.get('from_db') else ''
    ...
    if not all_jobs:
        empty_msg = 'No checks yet. <a href="/" style="color:var(--neon);text-decoration:none">Run your first checker &rarr;</a>' if email else \
                    'Sign in to see your saved history. <a href="/" style="color:var(--neon);text-decoration:none">Run a check &rarr;</a>'
        ...
        subtitle = 'Your check history — saved to your account.' if email else \
                   'Session history only. <a href="/account" style="color:var(--neon);text-decoration:none">Sign in</a> to save permanently.'
